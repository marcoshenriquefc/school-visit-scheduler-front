<script setup lang="ts">
  ...
  const loadUnits = async () => {
    ...
    errorMessage.value = "";
    try {
      units.value = await unitsService.list(filters);
    }
    catch (error) {
...
<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <h2 class="text-2xl font-semibold">Unidades</h2>
      <NuxtLink v-if="canManage" to="/admin/units/create">
        <BaseButton label="Nova unidade"/>
      </NuxtLink>
    </div>
    ...
    <BaseCard>
      <div class="grid gap-3 md:grid-cols-4">
        ...
        <div class="flex items-end">
          <BaseButton :loading="isLoading" label="Filtrar" @click="loadUnits" />
        </div>
      </div>
    </BaseCard>
    ...
    <div v-else-if="isLoading" class="flex items-center gap-2">
      <BaseLoading /> Loading units...
    </div>

    <BaseTable v-else-if="units.length">
      <thead>
      ...
      <tbody>
        <tr v-for="unit in units" :key="unit.id" class="border-b">
          ...
          <td class="p-2">{{ unit.identifier }}</td>
          <td class="p-2">{{ unit.defaultCapacityPerHour }}</td>
          <td class="p-2">
            <BaseBadge :variant="unit.isActive ? 'success' : 'danger'">
              {{ unit.isActive ? "Ativo" : "Inativo" }}
            </BaseBadge>
          </td>
          <td class="p-2 space-x-2">
            <NuxtLink :to="`/admin/units/${unit.id}/edit`">
              <BaseButton
                v-if="canManage"
                variant="secondary"
                label="Editar"
            ...
            <BaseButton
              v-if="canManage"
              :variant="unit.isActive ? 'danger' : 'success'"
              :label="unit.isActive ? 'Desativar' : 'Ativar'"
              @click="toggleStatus(unit.id, unit.isActive)"
            />
          </td>
        </tr>
      </tbody>
    ...
    <ConfirmDialog
      ...
      description="Deseja realmente desativar esta unidade?"
